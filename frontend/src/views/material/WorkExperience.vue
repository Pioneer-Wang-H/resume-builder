<template>
  <div class="page">
    <h2>工作/实习经历 <el-button type="primary" size="small" @click="openDialog()">添加</el-button></h2>
...
import { getWorks, saveWork, updateWork, deleteWork } from '@/api/material'
import { ElMessage, ElMessageBox } from 'element-plus'

const list = ref([])
const visible = ref(false)
const isEdit = ref(false)
const form = reactive<any>({ company: '', position: '', startDate: '', endDate: '', description: '' })
let editId: number | null = null

onMounted(async () => { await load() })
async function load() { const res: any = await getWorks(); list.value = res.data || [] }
...
async function handleSave() {
  try {
    if (isEdit.value && editId) await updateWork(editId, { ...form })
    else await saveWork({ ...form })
